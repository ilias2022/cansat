...
Continuously sends telemetry (temp + pressure + altitude) every second
...
from machine import Pin, I2C, SPI, RTC, reset
...
LAUNCH_DATETIME = (2026, 3, 11, 2, 13, 0, 0, 0)  # Wed 11 Mar 2026, 1:00 PM
# ──────────────────────────────────────────────────────────────

rtc = RTC()
rtc.datetime(LAUNCH_DATETIME)
print(f"RTC set to {LAUNCH_DATETIME[0]}-{LAUNCH_DATETIME[1]:02d}-{LAUNCH_DATETIME[2]:02d} "
      f"{LAUNCH_DATETIME[4]:02d}:{LAUNCH_DATETIME[5]:02d}:{LAUNCH_DATETIME[6]:02d}")
...
def next_csv_filename(prefix):
    """Find next available measurements_{prefix}_N.csv filename"""
    tag = f"measurements_{prefix}_"
    highest = 0
    try:
        for name in os.listdir('/'):
            if name.startswith(tag) and name.endswith('.csv'):
                try:
                    n = int(name[len(tag):-4])
                    if n > highest:
                        highest = n
                except ValueError:
                    pass
        ...
        pass
    return f"{tag}{highest + 1}.csv"

DATA_FILE = next_csv_filename("battery" if ON_BATTERY else "debug")
...
def get_datetime_str():
    """Get current date/time from RTC as ISO-style string"""
    t = rtc.datetime()
...
def init_csv():
    ...
    f = open(DATA_FILE, 'w')
    f.write("datetime,temperature_C,pressure_hPa,altitude_m\n")
    f.close()
    print(f"✓ CSV created: {DATA_FILE}")

def save_reading(dt_str, temp, pressure, altitude):
    """Append a single reading to CSV"""
    try:
        f = open(DATA_FILE, 'a')
        f.write(f"{dt_str},{temp:.2f},{pressure:.2f},{altitude:.2f}\n")
        f.close()
    except Exception as e:
...
try:
    while True:
        if telemetry_active:
            ...
            # Real date/time from RTC
            dt_str = get_datetime_str()

            reading_number += 1
            print(f"  #{reading_number}  {dt_str}  T={temp:.2f}°C  P={pressure:.2f} hPa  Alt={altitude:.1f} m")

            # Transmit over LoRa
            transmit_packet('R', f"{reading_number},{temp:.2f},{pressure:.2f},{altitude:.1f}")

            # Save locally
            save_reading(dt_str, temp, pressure, altitude)
    ...
    rx_thread_running = False
    time.sleep(2)
    print("Done. Safe to access filesystem.")