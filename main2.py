...
def save_reading(temp, pressure):
    """Append a single reading to file"""
    try:
        with open(DATA_FILE, 'a') as f:
            f.write(f"{time.time()},{temp:.2f},{pressure:.2f}\n")
    except Exception as e:
...
try:
    while True:
        if telemetry_active:
            ...
            reading_number += 1
            print(f"  #{reading_number}  T={temp:.2f}°C  P={pressure:.2f} hPa")

            # Transmit over LoRa
            transmit_packet('R', f"{reading_number},{temp:.2f},{pressure:.2f}")

            # Save locally
            save_reading(temp, pressure)
