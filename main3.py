...
def next_csv_filename():
    """Find next available measurements_N.csv filename"""
    highest = 0
    try:
        for name in os.listdir('/'):
            if name.startswith('measurements_') and name.endswith('.csv'):
                try:
                    n = int(name[13:-4])  # between 'measurements_' and '.csv'
                    if n > highest:
                        highest = n
                except ValueError:
                    pass
        ...
        pass
    return f"measurements_{highest + 1}.csv"

DATA_FILE = next_csv_filename()
